...
const BusNavigationInfo = ({ totalDuration, totalDistance, onStartNavigation }) => {
  ...
  const updateLocation = useCallback(async () => {
    try {
      ...
      const schedule = await getUpcomingShuttles(location.coords.latitude, location.coords.longitude);
            ...
            <>
              ...
              <View style={sheetStyles.tableBody}>
                {upcomingSchedule.upcoming_shuttles.map(item => (
                  <View key={`${item.scheduled_time}-${item.time_to_departure}`} style={sheetStyles.tableRow}>
