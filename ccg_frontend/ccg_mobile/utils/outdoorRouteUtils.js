...
export const getCumulativeDistances = coords => {
  ...
  for (let i = 1; i < coords.length; i++) {
    const d = calculateDistance(coords[i - 1][1], coords[i - 1][0], coords[i][1], coords[i][0]);
    total += d;
...
export const getCumulativeDistanceToNearestCoord = (coords, userLocation) => {
  ...
  for (let i = 0; i < coords.length; i++) {
    const d = calculateDistance(coords[i][1], coords[i][0], userLocation[1], userLocation[0]);
    if (d < minDistance) {
      minDistance = d;
      minIndex = i;
    }
