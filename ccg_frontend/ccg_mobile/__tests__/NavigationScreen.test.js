...
jest.mock('../utils/defaultLocations', () => ({
  getMyCurrentLocation: jest.fn(() => Promise.resolve({
    name: 'Current Location',
    civic_address: '1455 De Maisonneuve Blvd W',
    location: { latitude: 45.497, longitude: -73.579 }
  })),
...
const mockDirectionsResponse = {
  bbox: [45.490, -73.585, 45.505, -73.570],
  steps: [
    { points: [[45.497, -73.579], [45.498, -73.578]], instruction: 'Walk north', distance: 100, duration: 120 }
  ],
  total_distance: 100,
  total_duration: 120,
  legs: [
    { points: [[45.497, -73.579], [45.498, -73.578]] }
  ]
};
...
jest.mock('../api/dataService', () => ({
  ...
  getDirections: jest.fn(async (mode, start, end) => {
    return {
      bbox: [45.490, -73.585, 45.505, -73.570],
      steps: [
        { points: [[start[1], start[0]], [end[1], end[0]]], instruction: 'Walk to destination', distance: 100, duration: 120 }
      ],
      total_distance: 100,
      total_duration: 120,
      legs: [
...
describe('NavigationScreen', () => {
  ...
  it('uses provided start and destination points from route params', async () => {
    ...
    const customDestinationPoint = {
      ...
      location: { latitude: 45.494, longitude: -73.580 }
    };
  ...
  it('handles address modifications from NavigationHeader', async () => {
    ...
    const newDestination = {
      ...
      location: { latitude: 45.490, longitude: -73.585 }
    };
    ...
    expect(getDirections).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Array),
      [newDestination.location.longitude, newDestination.location.latitude]
    );
  });
