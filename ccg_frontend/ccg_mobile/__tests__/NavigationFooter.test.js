...
jest.mock('../components/navigation-screen-ui/sections/NavigationFooter', () => {
    ...
    return function MockNavigationFooter({ onStartNavigation, totalDuration, totalDistance }) {
        return (
            <View>
                <Text>{totalDuration ? parseFloat('' + totalDuration / 60).toFixed(2) + " minutes" : "Duration not available"}</Text>
                <Text>{totalDistance ? parseFloat('' + totalDistance / 1000).toFixed(2) + " km" : "Distance not available"}</Text>
