...
describe("useRouteInstruction", () => {
  ...
  it("should process route data when all inputs are valid", () => {
    ...
    expect(outdoorRouteUtils.getCurrentStepData).toHaveBeenCalledWith(mockRouteData, mockCurrentDistance);
  });
  ...
  it("should recalculate when userLocation changes", () => {
    const { result, rerender } = renderHook(props => useRouteInstruction(props.routeData, props.userLocation), {
    ...
    const newUserLocation = [-73.58, 45.498];
  ...
  it("should handle steps with missing coordinate data", () => {
    ...
    expect(outdoorRouteUtils.getCumulativeDistanceToNearestCoord).toHaveBeenCalledWith([], mockUserLocation);
  });
});
