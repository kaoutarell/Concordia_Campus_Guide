  ...
  return function MockBusNavigationInfo(props) {
    return (
      <View>
        <Text>
          {props.totalDuration
            ? parseFloat(props.totalDuration / 60).toFixed(2) + " minutes"
            : "Duration not available"}
        </Text>
        <Text>
          {props.totalDistance ? parseFloat(props.totalDistance / 1000).toFixed(2) + " km" : "Distance not available"}
        </Text>
  ...
  test("renders correctly with duration and distance", () => {
    ...
    const { getByText } = render(
      <View>
        ...
        <Text>
          {props.totalDistance
            ? parseFloat("" + props.totalDistance / 1000).toFixed(2) + " km"
