...
class BusLocationService {
  ...
  constructor() {
    if (BusLocationService.instance) {
      throw new Error(
        'Use BusLocationService.getInstance() instead of new BusLocationService().'
      );
    }
  ...
  async fetchSessionCookie() {
    try {
      ...
      console.error(
        'BusLocationService: Error fetching session cookie:',
        error
      );
      return null;
    }
  ...
  async fetchBusData() {
    try {
      ...
      if (Array.isArray(data?.d?.Points)) {
        const busPoints = data.d.Points.filter((point) =>
