...
class ShuttleViewsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.shuttle_schedule5 = ShuttleSchedule.objects.create(campus="SGW", day_of_week=0, time=time(16, 30))
        self.shuttle_schedule6 = ShuttleSchedule.objects.create(campus="SGW", day_of_week=0, time=time(17, 0))
        self.shuttle_schedule7 = ShuttleSchedule.objects.create(campus="SGW", day_of_week=0, time=time(17, 30))
        self.shuttle_schedule8 = ShuttleSchedule.objects.create(campus="SGW", day_of_week=0, time=time(18, 0))
        self.shuttle_schedule9 = ShuttleSchedule.objects.create(campus="SGW", day_of_week=0, time=time(18, 30))
        # mock datetime.now().time() to 14:00:00
        self.patcher = patch('mapengine.views.shuttle_views.datetime')
        self.mock_datetime = self.patcher.start()
        self.mock_datetime.now.return_value = datetime(2025, 2, 10, 14, 0, tzinfo=self.tz)  # Mocked to 14:00:00
    ...
    def test_get_upcoming_sheduled_shuttle_missing_coord(self):
        ...
        self.assertEqual(response.json()['upcoming_shuttles'][0]['scheduled_time'], "14:30")
