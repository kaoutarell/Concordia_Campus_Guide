...
def test_parse_coordinates_invalid_format():
    ...
    with pytest.raises(
        InvalidCoordinatesError, match="Invalid coordinate format. Use 'lon,lat'."
    ):
...
def test_find_nearest_building_not_found():
    ...
    with pytest.raises(
        BuildingNotFoundError, match="No nearby building found for the given location."
    ):
...
def test_get_shuttle_stops_not_found():
    ...
    with pytest.raises(
        ShuttleStopNotFoundError,
        match="Shuttle stop not found for one or both campuses.",
    ):
...
def test_build_combined_route():
    ...
    route_legs = {
        "walk_to_stop": {
            "total_distance": 100,
            "total_duration": 10,
            "steps": [{"instruction": "Walk forward"}],
        },
        "shuttle_ride": {
            "total_distance": 500,
            "total_duration": 30,
            "steps": [{"instruction": "Board shuttle"}],
        },
        "walk_from_stop": {
            "total_distance": 200,
            "total_duration": 20,
            "steps": [{"instruction": "Walk to building"}],
        },
    }
